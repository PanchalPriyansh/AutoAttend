...
- **A component rendered by more than one page gets its own stylesheet, not a home in any page's file.** There are two: `faculty-roster-row.css` holds `StudentStatusRow`, which both faculty attendance pages render, and `confirm-dialog.css` holds `ConfirmDialog`, which six files across admin and faculty render. Neither is a page or a primitive: keeping the row in `faculty-attendance.css` meant the history page's rows were styled by a file named after a different page, and `components.css` is for patterns with two *independent* implementations, whereas these are single components used several times — which is also why the vocabulary still has no `.modal`. The split is strict: the component file owns what belongs to the component, and each page owns how it *arranges* them (`.fa-roster`, `.fh-roster`). Load a component file before every page that renders it, so any of them can override with a plain later selector.
...
**Next: the polish pass.** Agreed 2026-08-26 — small improvements to the site now that the redesign is done, **not yet scoped: ask before starting it, and do not assume its contents.** What has accumulated for it, in rough order of substance:

- **`ConfirmDialog` has no focus trap** though it declares `aria-modal="true"`, so Tab escapes to the page behind. Left deliberately: it is behaviour across six screens, not style. Its stylesheet records this.
- **`--text-muted` on `--accent-weak` measures 4.47:1 light / 4.49:1 dark**, under the contrast floor. It ships today in `faculty-history.css` (`.fh-session-facts` inside `.fh-session--open`); `/admin/users` avoided it by moving one element to `--text`.
- **The `.card--link` extraction** — `/student`, `/faculty` and `/admin` each render a card whose whole area is one nav target, ~90 identical lines each. The set is closed, so extracting later costs nothing, and it is not mechanical: `/student` would have to move onto the other two's gutter (56/24 → 48/20) and breakpoint (480 → 420). Open: whether it belongs in `components.css` (it would be the first primitive built from a *coordinated set* of classes joined by `:has()`) or in a `styles/portal-card.css` on the `faculty-roster-row.css` precedent.
- **Shell items deferred on purpose** — a collapsible nav for the still-three-row mobile header, a spacing scale, a manual theme toggle.
