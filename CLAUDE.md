...
**Next: `25`, forgot-password, delivered as an emailed one-time code.** The last piece of password management, and the only planned spec outstanding. `23` shipped the authenticated half, `24` closed the token-invalidation decision `05` recorded and `23` inherited — so the two things that used to block it are gone, and what remains is genuinely its own feature. The code **resets the password** rather than signing the user in, so the OTP *is* the forgot-password feature and there is no separate link-based reset to build first; the alternatives were rejected for adding a permanent second auth path or a half-authenticated token state. It needs a store for the code — hashed, TTL'd, single-use, with an `attempts` cap, since a 6-digit code is 10⁶ and **this project has no rate limiting anywhere** — plus a per-address send cooldown against mail-bombing, and an always-`200` generic response so it cannot become an account-enumeration oracle (login already burns a dummy hash for exactly that reason). It is also the **first mail on a request path**: `10` and `11` made mail CLI-only deliberately and `test_app_factory.py` still asserts no route contains `notification`, so the spec must state that departure out loud, including that `smtplib` is synchronous and will hold a worker.
...
- **Forgot-password is no longer on this list** — it is scheduled as `25`, described under "Next planned feature" above. Everything `23` deferred in turn (no rate limiting, no password history, no email confirming a change, no admin-visible record) is in its own row above, and what `24` deferred (no per-request session check, no per-device session list, no access-token blocklist) is in the `Authentication` row.
