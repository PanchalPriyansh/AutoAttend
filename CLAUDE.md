...
**Next: unscheduled.** The deferred items below are what remains, none of them picked. Of the two named as likely candidates before `21`, one is now half-done: `23` shipped the **authenticated** half of password management, and **forgot-password is what is left of it** — still the larger piece, and still unpicked. It needs a reset-token store (a new collection, expiry, single-use semantics), it needs an email path on a *request* thread when `10` and `11` deliberately made mail CLI-only and `test_app_factory.py` still asserts no route contains `notification`, and it needs the token-invalidation decision `05` recorded and `23` inherited rather than solved. The other candidate is unchanged: a faculty/admin view of a *named* student's attendance, ruled out as "small" — see below.
...
- **Forgot-password** — what is left of "password management" now that `23` has shipped the authenticated half. Still the larger piece, and the reason the two were split rather than specced together: it needs a reset-token store, an email path on a request thread against `10`/`11`'s deliberately CLI-only mail, and the token-invalidation decision `05` recorded and `23` inherited. Everything `23` deferred in turn (no rate limiting, no password history, no email confirming a change, no admin-visible record) is in its own row above.
