...
Specs `01`–`10` are built, so every core capability this file describes now exists. The agreed next step is a single small spec:

**`11-student-attendance-threshold`** — show a student the bar they have to clear. `10` closed the loop by *emailing* a student that they are below the required percentage, but the dashboard it tells them to check has never displayed that percentage; a student at 78% is fine today and has no way to see they are two lectures from trouble.

Deliberately small and additive: `LOW_ATTENDANCE_THRESHOLD` already exists in `config.py`, `GET /api/attendance/me` already computes every figure, and the student visuals already exist. Expected shape is a `threshold` field added to the two existing student responses, a marker line on the percentage bars already drawn at `/student/attendance`, a met/below label per class, and a "attend N more lectures to reach the bar" figure derived from numbers already on hand. **No new collection, no new endpoint, no new dependency, no new page.**

The same wording discipline `notifications/messages.py` follows applies to anything shown on screen: it states a recorded figure against a configured bar. It must not become a projection of a final percentage, a prediction, or a statement of consequence.

Two alternatives were considered and not chosen: an admin trigger with an admin-set threshold (larger — new blueprint, admin route, threshold moves to MongoDB, and it would undo the deliberately-kept `"notification"` route guard), and CSV export. A faculty/admin view of a *named* student's attendance was deliberately ruled out as a "small" feature: `09` built things so that **no endpoint anywhere takes a student id**, and undoing that is a security decision, not a finishing touch.
