...
frontend/src/styles/
  ...
  password-field.css          .pw-*  (one component, rendered by the
                              SEVEN password boxes on /login,
                              /forgot-password, /account and
                              /admin/users)
  auth-screen.css             .auth-*  (the signed-out screen,
                              rendered by BOTH /login and
                              /forgot-password -- was login.css)
  forgot-password.css         .fp-*
...
**`auth-screen.css` is the same rule arriving at a screen rather than a
component.** It was `login.css` until `25-forgot-password` gave the
project a second signed-out page; both render the whole of it, so it
belongs to neither, and it sits before both so either can override with
a plain later selector. The rename moved **51 declarations and changed
none of them** (proved by stripping comments from both files and
diffing) and added exactly one rule, `.auth-forgot`, for the link that
motivated the rename — which costs `/login` 42.1px of card height and
re-centres every box above it by half that, with no other geometry
moving at all.
...
- **A component rendered by more than one page gets its own stylesheet, not a home in any page's file.** There are four: `faculty-roster-row.css` holds `StudentStatusRow`, which both faculty attendance pages render; `confirm-dialog.css` holds `ConfirmDialog`, which six files across admin and faculty render; `portal-card.css` holds `PortalCard`, which all three landing pages render; and `password-field.css` holds `PasswordInput`, which the seven password boxes on `/login`, `/forgot-password`, `/account` and `/admin/users` render. None is a page or a primitive: keeping the row in `faculty-attendance.css` meant the history page's rows were styled by a file named after a different page, and `components.css` is for patterns with two *independent* implementations, whereas these are single components used several times — which is also why the vocabulary still has no `.modal`. The split is strict: the component file owns what belongs to the component, and each page owns how it *arranges* them (`.fa-roster`, `.fh-roster`, and the three portal grids). Load a component file before every page that renders it, so any of them can override with a plain later selector. **`PortalCard` is the one that was extracted late rather than early**, and it is the cautionary case: three pages each built the same control on their own hooks, each wrote in its own comments that all three should look identical, and by extraction time they had drifted on four values — a gutter, a chevron offset, a breakpoint and the description's type — including a raw `12px` that no longer read as the `--radius-md` it was. Three copies is where a fourth comes from.
...
them, and so have `20`–`24`, and they earned their place: the review pipeline
caught a real provenance bug in `08`, three accepted hardening changes in `10`,
and four accepted refinements in `20`.

**The yield is falling, and that is worth watching rather than acting on yet.**
`22` returned no high- or medium-severity findings and `24` returned none from
either reviewer — its two applied items were both cosmetic. That is not an
argument for skipping the pipeline on the next backend slice: `24`'s real defect
was caught *before* review, by reading the test double and by checking the BSON
type against a live server, and neither is something a diff reader would have
found. What it suggests is that the pipeline now mostly confirms rather than
discovers, so the discovering has to happen somewhere else — which is what the
"prove it, do not assume it" habit in `18`, `19` and `23` is for.
...
| Authentication | Implemented — Flask-JWT-Extended login/refresh/logout/me via HttpOnly cookies, role-based `@role_required`, `flask create-admin` bootstrap. No public registration. **`24-invalidate-tokens-on-password-change` made a session outlive its credential impossible.** `users.token_version` is an integer bumped every time a password is written; login stamps it into the **refresh** token, and `POST /api/auth/refresh` refuses a token whose stamp no longer matches. Before it, refresh re-checked only `is_active` — so a refresh cookie, which lives **seven days**, kept minting access tokens for a week after the owner changed their password, and the "15 minutes" `05` and `23` both quoted described the access token alone. The refusal is deliberately indistinguishable from the deactivated and deleted cases (same `401`, same body), so it cannot tell a holder of stolen cookies why they were stopped. The rule is a pure module, `auth/tokens.py` — no Flask, no pymongo, no config, no imports at all — because a five-line comparison that decides whether a week-old cookie still authenticates somebody should be readable and testable without a database. **Absent means zero on both sides**: a document written before the feature and a token minted before it both read as `0` and therefore match, so deploying it signs nobody out; that account's next password change strands them, which is the moment they should be stranded. Comparison is exact integer equality, never `>=` and never a timestamp (a date would have to survive BSON millisecond truncation against JWT float seconds, then be compared with an inequality that accepts a token minted in the same second as the change). Deferred on purpose: **no per-request check**, so a stolen *access* token stays usable until it expires — genuinely ≤15 minutes now — because verifying on every request means a database read on every authenticated route, which is a change to the auth model of every feature rather than part of this one; the access token therefore carries **no** version claim, since a claim nothing reads looks like a guarantee it is not. Also deferred: no per-device session list or "sign out everywhere" control, no blocklist for an access token, no notification that other sessions ended, and no record of when a password last changed. |
| Change password | Implemented — `POST /api/auth/password`, the first endpoint in the project open to **any** signed-in role (`@jwt_required()`, never `@role_required`): an admin, a faculty member and a student have equal standing over their own credential. **No user id is accepted anywhere** — not in the path, not in the body — so the account written is the one the JWT identifies and there is nothing to tamper with (`09`'s rule, applied to a credential); a body carrying `user_id`/`id`/`email` is simply never read. It lives on `auth_bp` rather than `users_bp` because every route in `routes/users.py` is `@role_required("admin")` and that uniformity is worth reading once instead of line by line, and it is a `POST` rather than a `PUT` because it consumes a proof — replay it and it fails. **A wrong current password is `403`, not `401`, and that is load-bearing rather than stylistic**: `apiFetch` transparently refreshes and *retries* any non-login 401, so a 401 would submit the same wrong password twice on every attempt — confirmed in the browser, where the failing case issues exactly one POST and no refresh. `is_active` is re-read from the database rather than trusted from the token claim, closing `05`'s 15-minute window against a deactivated account setting a new password and walking back in. `MIN_PASSWORD_LENGTH` applies to the **new** password only — length-checking the current one would lock out exactly the people whose stored password is now too short — and neither is trimmed. A no-op change is a `400`, not a silent success. Logic sits in `auth/password_change.py` (pure: no Flask), which is its own module because `users/service.py` already imports `auth/service.py` and the reverse would be a circular import; the write delegates to `set_user_password` unwrapped, so a self-service change is byte-for-byte what an admin reset writes. `auth/errors.py` is new, following the per-package pattern. There is **no logging on the path at all** — the surest way never to write a password to a log file. UI at `/account`, for all three roles, reached from the user's own name in the header: `navigation.js` is deliberately untouched, because a link there appears on six surfaces, takes the student nav off `.app-nav--single` (which `17` measured as 17px *worse*) and invalidates the 413px breakpoint `17` derived from faculty's list. The name became a `<Link>` rather than gaining a control beside it, since `19` showed `.app-user` has no slack — **re-measured across 38 role×width cells with zero drift**, the 413/414 collapse boundary and student's 599 one-row threshold both exactly intact, and a 25-character name still ellipsing while "Log out" holds at 81.8px. Deferred: **forgot-password was a separate spec and is now shipped as `25`** (see its own row below), no rate limiting (the project has none anywhere, and this only lowers the cost of confirming a password for someone who already holds a session), no password-history or complexity rules beyond the length minimum, no email confirming a change, and no admin-visible record that it happened. **Its token-invalidation deferral is closed** — `24` ended every other session on the account, and this endpoint now re-issues the acting session's refresh cookie in the same `200` so the caller stays signed in; without that the bump would have stranded the changer too, silently, fifteen minutes later. The response body is unchanged, and the access cookie is deliberately not re-issued (nothing invalidated it). |
| Forgot password | Implemented — unauthenticated `POST /api/auth/forgot-password` and `POST /api/auth/reset-password`. A signed-out user asks for a 6-digit code by email and submits it with a new password; **the code resets the password and does not sign anybody in**, so there is no second authentication path beside `/login` and no half-authenticated token state — the user then logs in normally, and `24`'s bump means the reset has already ended every other session on the account, which is right for somebody who may be recovering from a compromise. `email` is required beside the code, because matching a code alone would turn one guess in 10⁶ into a birthday problem across every outstanding code at once. **The request endpoint is not an enumeration oracle in any dimension**: a real address, an unknown one, a deactivated account, an address inside the send cooldown, and a *failed send* all get the same `200` and the same body — `load_smtp_settings` is called **before** the user lookup so even the misconfigured-server `503` cannot depend on the address. Every failure of a code is one `400` with one message covering six outcomes; **`400` and never `401`**, because `apiFetch` retries a non-login 401 and here the retry would spend a second of the five attempts (`23`'s finding, with sharper teeth). **This project has no rate limiting anywhere**, so the compensating controls are the feature: the code is hashed at rest through `auth/passwords.py`, expires in 15 minutes, is single-use via an atomic update conditional on `consumed_at` being absent, carries a 5-attempt cap, and is paced by a 60-second per-account send cooldown. **Both caps are enforced atomically, and `/code-review-feature` is why.** The first cut checked the cooldown with a read and wrote the row after the SMTP send, so concurrent requests all passed the check and all sent mail — a cooldown that concurrency walks past is not a control, and this endpoint points at somebody else's inbox. Issuing is now ONE upserting `find_one_and_update` whose filter matches only a row older than the cooldown; a row inside it makes the filter miss, the upsert falls through to an insert, and the **unique index on `password_reset_codes.user_id`** refuses it. That index also makes "one live code per account" structural — two live codes would be two independent attempt budgets. The attempts cap is charged in the same update that selects the code, so five bounds *guesses* rather than requests-in-flight. **The write now happens before the send**, reversing `10`'s send-then-record rule, and the price is stated rather than hidden: a failed send leaves a stored undelivered code and keeps the cooldown slot, deliberately not rolled back, because releasing it is the one thing that must not be cheap. **This is the first mail this project sends from a request thread** — `10` and `11` made mail CLI-only on purpose, and `test_app_factory.py`'s `notification` guard still passes and still means what it says, but now means "the low-attendance *sweep* has no HTTP trigger" rather than "no route sends mail"; its comment says so. `smtplib` is synchronous and holds a worker for one connection and send, bounded by `SmtpTransport`'s 30-second timeout. Nothing on either path is logged that identifies who is resetting what. The rule lives in `auth/reset_codes.py` (pure: no Flask, pymongo, bson or config), which expresses expiry, consumption, attempts and cooldown as **query filters** rather than Python predicates — pymongo returns naive datetimes and comparing one to a fresh `now` raises `TypeError`, a bug this project has already shipped once (`notifications/service.py::cooldown_skips`), **re-confirmed against a live server here**. Orchestration is `auth/password_reset.py`, the email copy `notifications/reset_messages.py` (plain text, no HTML part, **no link**, code in the body and never the subject). UI at `/forgot-password`, signed out, three stages in one component with the code and password submitted together. Deferred: **no rate limiting** (the attempts cap, the cooldown and the expiry are the whole of the control), no TTL index and so no sweep of expired rows — `database/indexes.py`'s planner from `20` compares `keys` and `unique` only, so an `expireAfterSeconds` index would be silently rebuilt *without* its TTL on the next `flask init-db`, which is worse than none; expiry is enforced in the query instead and a spent row is deleted. Also deferred: no queue or background send, the residual timing difference between a known and an unknown address (a real send costs an SMTP round trip and nothing else does), no notification that a password was reset, no admin-visible record that one happened, and no lockout after repeated requests for one address. |
...
| Admin user management | Implemented — admin-only `/api/users` CRUD (create/list/get/update, `PUT .../status`, `PUT .../password`), plus `PUT /api/classes/<id>/faculty` and `/api/classes/<id>/students` for enrollment. Accounts are deactivated, never deleted; `role` is immutable after creation; an admin cannot deactivate themselves or the last active admin. Admin UI at `/admin/users`, with the class assignment panel inside `/admin/academics`. **A reset now ends the target's other sessions**, and `routes/users.py` carries no code for it: `24` put the `token_version` bump inside `set_user_password` — the one function in the project that writes a password — so the admin reset and the self-service change cannot drift apart. It re-issues nothing, correctly: it acts on a named user who is normally not the caller, and an admin who points it at their own account is resetting their own password, where being signed out everywhere is the right outcome. Deferred: pagination on the user list, and invalidating an already-issued *access* token on reset or deactivation — now genuinely bounded by the 15-minute access-token lifetime rather than by the refresh cookie's seven days. |
...
**The bump went where the write already was.** Putting `$inc` inside `set_user_password`, in the same update as the `$set`, meant the admin reset inherited the behaviour with zero lines of its own and cannot drift from the self-service path — `23`'s "delegate the write, unwrapped" rule paying for itself a spec later. One update rather than two also means no window where the new hash is live and the old tokens still are.

**The naive version of this feature signs the caller out.** The bump strands the changer's own refresh token along with everybody else's, and the failure is invisible for fifteen minutes before dropping them at `/login` for the crime of changing their password. `POST /api/auth/password` therefore re-issues the refresh cookie in the same `200`. Both halves were verified together in one browser run: the changing session survived an access-token expiry and kept working, while a session established beforehand was refused at its very next refresh.

**The test double was the load-bearing test change, and it was not in the spec's file list.** `auth_test_helpers.py`'s fake modelled `$set` only, with a docstring asserting nothing used another operator. Left alone it would have silently dropped every `$inc`, so the version would never have moved and the assertions about it would have passed anyway. Reading the fake before trusting the suite is the habit to keep; a fake that has drifted from the code it stands in for is worse than no fake.

**`23-change-password` is the one before it.** A backend slice with a user-facing half, so it took the full pipeline *and* browser measurement, like `21` and `22`. No dependency, no database change, no new collection — it is the smallest vertical slice since `20`, and deliberately so: it is **half** of "password management", and the half that needs nothing new. Splitting it that way is the point, and the reason it shipped in one session while forgot-password is still unstarted.

Three things about it are worth carrying forward. **A status code was chosen from the client's behaviour, not from taste**: a wrong current password is `403` because `apiFetch` transparently refreshes and retries any non-login `401`, so the intuitive answer would have submitted the wrong password twice per attempt — the browser run confirmed exactly one POST and no refresh, and that check belongs in the DoD of anything else that ever returns 401 from a non-login route. **The module boundary was forced by an import cycle**, not chosen for neatness: `users/service.py` already imports `auth/service.py`, so the logic went to `auth/password_change.py` rather than into the service beside the code it calls. **And the header change was verified by an A/B against the markup it replaced** — the link swapped back to a bare `<span>` in the live DOM, 38 role×width cells, zero drift — with a negative control that fires: strip the three ellipsis declarations and 320/360/413 overflow horizontally. The drift metric alone was *not* sensitive to that; the overflow flag was, which is the argument for measuring more than one signal.

**`22-bulk-face-enrollment-import` is the one before it.** A backend slice with a user-facing half, so it took the full pipeline *and* browser measurement, like `21`. It added no dependency and changed no collection: a bulk-imported sample is byte-for-byte what the single-upload path already wrote, and the `source` enum deliberately did **not** grow an `import` value, because how the admin operated the UI is not a fact about where the biometric data came from.
...
- **Forgot-password is off this list** — `25` shipped it, and what it deferred in turn (no rate limiting anywhere, no TTL sweep of expired code rows and why, no background send, the residual timing difference between a known and an unknown address, no notification or admin-visible record of a reset, no per-address lockout) is recorded in its own row above. Everything `23` deferred (no password history, no email confirming a change, no admin-visible record) is in its row, and what `24` deferred (no per-request session check, no per-device session list, no access-token blocklist) is in the `Authentication` row.
- **Rate limiting is now the most-cited absence in this file.** `23`, `24` and `25` each recorded it separately, and `25` had to build three compensating controls around it. It is still not scheduled, and it would be a project-wide concern rather than one feature's — but it is no longer a footnote.
